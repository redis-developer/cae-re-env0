...
class EnvironmentApiReviewerUser(BaseModel):
    """
    EnvironmentApiReviewerUser
    """ # noqa: E501
    user: User
    action: StrictStr
    __properties: ClassVar[List[str]] = ["user", "action"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['cancelled', 'approved']):
            raise ValueError("must be one of enum values ('cancelled', 'approved')")
        return value
